...
class Trilateration {
    ...
    compute(beacons) {
        ...
        const Vx = mathjs_1.dot(ex, mathjs_1.subtract(P3, P1));
        // transform to get circle 2 on x axis
        let ey = mathjs_1.subtract(P3, P1);
        ey = mathjs_1.divide(mathjs_1.subtract(ey, mathjs_1.multiply(Vx, ex)), mathjs_1.norm(mathjs_1.subtract(ey, mathjs_1.multiply(Vx, ex))));
        var ez = mathjs_1.cross(ex, ey);
        var U = mathjs_1.norm(mathjs_1.subtract(P2, P1));
        const Vy = mathjs_1.dot(ey, mathjs_1.subtract(P3, P1));
        // (r₁)² = x² + y² + z²
    ...
    levenbergMarquardtTri(beacons, initialValues) {
        ...
        const weight = [1]; // TBD: Check with the new method,
        ...
        let minValues = []; // n-vector of lower bounds for parameter values
        let maxValues = []; // n-vector of upper bounds for parameter values
        if (globalBBox) {
        ...
        let distanceFx = (independentVariable, prediction, parameters) => {
            ...
            for (var i = 0; i < numRows; i++) {
                ...
                let from = turf.point(this.ECEFLatLon(independentVariable[i][0], independentVariable[i][1], independentVariable[i][2]).slice(0, 2));
                result[i][0] = turf.distance(from, to, { units: "kilometers" }) * 1000;
            }
            return result;
        };
        var estimate = LM.optimize(distanceFx, initialGuess, independentVar, distances, weight, SME, minValues, maxValues, constants, opts).p;
